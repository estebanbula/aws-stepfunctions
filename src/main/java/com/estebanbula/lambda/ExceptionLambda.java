...
public class ExceptionLambda implements RequestHandler<InputObject, StepFunctionObject> {
    ...
    public StepFunctionObject handleRequest(InputObject input, Context context) {
        ...
        StepFunctionObject StepFunctionObject = new StepFunctionObject();

        if (bucket.get() != null && key.get() != null && !bucket.get().isEmpty() && !key.get().isEmpty()) {
            StepFunctionObject.setBucket(bucket.get());
            StepFunctionObject.setKey(key.get());
        } else {
        ...
        StepFunctionObject.setValidate(r);

        return StepFunctionObject;
    }
}
